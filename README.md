...
Setting `NOMODULE` to any non-blank value omits creation of the modulefile. That's good for when you build your own compiler or MPI: those modules are better written by hand since they need to inherit, declare `family`, et cetera.
...
```
DEPENDSON = whatever someother
## or:
DEPENDSON = whatever/1.1.1 someother/5.7
```
...
For cases like Petsc that have version 3.22.1 but module 3.22, use `DEPENDSONCURRENTMINOR`.

Important restriction: you can have only one `DEPENDS....` clause.
