...
The `depends_on` clause is generated by the `DEPENDSON` variable:
...
to pick up whatever current version the module is being built with. This looks for a variable `TACC_<whatever>_VERSION`. If this is not defined the version dependency is skipped.
